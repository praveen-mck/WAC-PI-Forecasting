...
--BEFORE
SELECT COUNT(*) FROM uspd_analytics_den.analytics_gold.WAC_PI_FORECAST_BASELINE_PRICE_V2_FINAL_MODIFIED_0622
--410936
...
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_RUNS_v12 AS
SELECT
...
/* >>>>>>>>>>>>>>>>>>>> CHANGED 07/13: NEW LIVE FORECAST RUN <<<<<<<<<<<<<<<<<< */
UNION ALL
...
--select count(distinct MTRL_NUM) from uspd_analytics_den.analytics_gold.WAC_PI_BT_UNIVERSE_v12
--10325
/* ---------------------------------------------------------------------
   ...
   --------------------------------------------------------------------- */
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_UNIVERSE_v12 AS
WITH base AS (
    SELECT
...
/* ---------------------------------------------------------------------
   ...
   --------------------------------------------------------------------- */
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BF_ATTRS_v12 AS
WITH
mtrl AS (
    SELECT
        MATERIAL,
...
FROM uspd_analytics_den.analytics_gold.WAC_PI_BT_UNIVERSE_v12 u
...
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BF_ATTRS_UPD_v12 AS
WITH
EP as (
    select 
        distinct 
...
from
    uspd_analytics_den.analytics_gold.WAC_PI_BF_ATTRS_v12 attrs_v1
left join
    EP
ON
...
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_ATTRS_DEDUP_v12 AS
WITH ranked AS (
    SELECT
        a.*,
        ROW_NUMBER() OVER (
    ...
    FROM uspd_analytics_den.analytics_gold.WAC_PI_BF_ATTRS_UPD_v12 a
)
SELECT
    ndc_nmbr,
    mtrl_num,
...
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_ACTUAL_WAC_MONTHLY_v12 AS
WITH base AS (
    SELECT
    ...
    JOIN
        uspd_analytics_den.analytics_gold.WAC_PI_BT_UNIVERSE_v12 mat_univ
    ON
...
===================================================================== */
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_MTRL_HISTORY_PROFILE_v12 AS
SELECT
    aw.ndc_nmbr,
    aw.mtrl_num,
...
FROM 
    uspd_analytics_den.analytics_gold.WAC_PI_BT_ACTUAL_WAC_MONTHLY_v12 aw
GROUP BY 1,2
;
...
===================================================================== */
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_ACTIVE_PRODUCTS_v12 AS
...
/* =====================================================================
MATERIAL LIFECYCLE TABLE (v12)
- Uses ACTUAL_WAC_MONTHLY_v12 (clean monthly WAC)
- One row per material
===================================================================== */

CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_MTRL_LIFECYCLE_v12 AS
WITH base AS (
    SELECT
        mtrl_num,
        ndc_nmbr,
        cal_month_start_dt,
        actual_wac
    FROM uspd_analytics_den.analytics_gold.WAC_PI_BT_ACTUAL_WAC_MONTHLY_v12
),
...
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_RUN_ELIGIBILITY_v12 AS
WITH base AS (
    SELECT
        r.run_id,
        r.jump_off_month,
    ...
    FROM uspd_analytics_den.analytics_gold.WAC_PI_BT_RUNS_v12 r
    JOIN uspd_analytics_den.analytics_gold.WAC_PI_BT_UNIVERSE_v12 u
      ON 1 = 1
    LEFT JOIN uspd_analytics_den.analytics_gold.WAC_PI_BT_MTRL_LIFECYCLE_v12 lc
      ON u.ndc_nmbr = lc.ndc_nmbr
     AND u.mtrl_num = lc.mtrl_num
    LEFT JOIN uspd_analytics_den.analytics_gold.WAC_PI_BT_ACTIVE_PRODUCTS_v12 ap
...
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_HIST_v12 AS
SELECT
    re.run_id,
    re.jump_off_month,
    re.history_start_dt,
...
FROM uspd_analytics_den.analytics_gold.WAC_PI_BT_RUN_ELIGIBILITY_v12 re
JOIN uspd_analytics_den.analytics_gold.WAC_PI_BT_ACTUAL_WAC_MONTHLY_v12 aw
...
===================================================================== */
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_HIST_AGE_v12 AS
SELECT
    re.run_id,
    re.mtrl_num,
...
FROM uspd_analytics_den.analytics_gold.WAC_PI_BT_RUN_ELIGIBILITY_v12 re
LEFT JOIN uspd_analytics_den.analytics_gold.WAC_PI_BT_HIST_v12 h
...
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_LAST_WAC_v12 AS
SELECT
    re.run_id,
    re.mtrl_num,
...
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_PRICE_CHANGES_v12 AS
SELECT
    z.*,
...
FROM (
    ...
    FROM uspd_analytics_den.analytics_gold.WAC_PI_BT_HIST_v12 h
) z
;
...
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_VALID_INCREASE_EVENTS_v12 AS
SELECT *
FROM uspd_analytics_den.analytics_gold.WAC_PI_BT_PRICE_CHANGES_v12
WHERE 
...
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_EVENT_PROFILE_v12 AS
SELECT
    re.run_id,
    re.mtrl_num,
...
FROM uspd_analytics_den.analytics_gold.WAC_PI_BT_RUN_ELIGIBILITY_v12 re
LEFT JOIN uspd_analytics_den.analytics_gold.WAC_PI_BT_PRICE_CHANGES_v12 pc
...
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_LAST_INCREASE_v12 AS
SELECT
    run_id,
    mtrl_num,
    MAX(cal_month_start_dt) AS last_increase_dt
FROM uspd_analytics_den.analytics_gold.WAC_PI_BT_VALID_INCREASE_EVENTS_v12
GROUP BY 1,2
;
...
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_NDC_MAGNITUDE_v12 AS
...
loe_stats AS (
    ...
    FROM uspd_analytics_den.analytics_gold.WAC_PI_BT_PRICE_CHANGES_v12 pc
    LEFT JOIN uspd_analytics_den.analytics_gold.WAC_PI_BF_ATTRS_UPD_v12 loe
...
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_NDC_TIMING_v12 AS
...
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_THERAP_FALLBACK_v12 AS
WITH material_level AS (
    ...
    FROM uspd_analytics_den.analytics_gold.WAC_PI_BT_RUN_ELIGIBILITY_v12 re
    JOIN uspd_analytics_den.analytics_gold.WAC_PI_BT_ATTRS_DEDUP_v12 ad
      ON re.ndc_nmbr = ad.ndc_nmbr
     AND re.mtrl_num = ad.mtrl_num
    LEFT JOIN uspd_analytics_den.analytics_gold.WAC_PI_BT_NDC_MAGNITUDE_v12 nm
      ON re.run_id = nm.run_id
     AND re.mtrl_num = nm.mtrl_num
    LEFT JOIN uspd_analytics_den.analytics_gold.WAC_PI_BT_NDC_TIMING_v12 nt
...
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_MFR_FALLBACK_v12 AS
WITH material_level AS (
    SELECT
        re.run_id,
...
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_FALLBACK_ENRICHED_v12 AS
WITH base_materials AS (
    SELECT DISTINCT
        run_id,
        mtrl_num,
        ndc_nmbr
    FROM uspd_analytics_den.analytics_gold.WAC_PI_BT_HIST_v12
)
...
JOIN ATTRIBUTES
=============================== */
LEFT JOIN uspd_analytics_den.analytics_gold.WAC_PI_BT_ATTRS_DEDUP_v12 a
...
=============================== */
LEFT JOIN uspd_analytics_den.analytics_gold.WAC_PI_BT_THERAP_FALLBACK_v12 tf
...
=============================== */
LEFT JOIN uspd_analytics_den.analytics_gold.WAC_PI_BT_MFR_FALLBACK_v12 mf
...
- Uses FALLBACK_ENRICHED_v12 only for NEW products (<12 months)
- Applies overrides with LOE as highest priority
- Adds stale timing override for non-new products
===================================================================== */

CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_BASELINE_RESOLVED_v12 AS
...
joined AS (
    ...
    LEFT JOIN uspd_analytics_den.analytics_gold.WAC_PI_BT_NDC_MAGNITUDE_v12 ms
        ON bm.run_id = ms.run_id
       AND bm.mtrl_num = ms.mtrl_num
    LEFT JOIN uspd_analytics_den.analytics_gold.WAC_PI_BT_NDC_TIMING_v12 nt
        ON bm.run_id = nt.run_id
       AND bm.mtrl_num = nt.mtrl_num
    LEFT JOIN uspd_analytics_den.analytics_gold.WAC_PI_BT_FALLBACK_ENRICHED_v12 fe
        ON bm.run_id = fe.run_id
       AND bm.mtrl_num = fe.mtrl_num
    LEFT JOIN uspd_analytics_den.analytics_gold.WAC_PI_BT_ATTRS_DEDUP_v12 attr
...
---------------------------------------------------------------------
BEFORE: forecast months came ONLY from ACTUAL_WAC_MONTHLY_v12, so a
        month existed only if an actual existed. Horizon = current month.

AFTER : - BT_ runs: identical logic to your current code, untouched.
        - FC_LIVE : a generated 60-month spine (current month .. +59),
                    independent of whether actuals exist.

NEW COLUMNS:
    month_type      'ACTUAL_MONTH' (backtest) | 'FUTURE_MONTH' (projection)

FC_LIVE is deliberately EXCLUDED from the actual_months branch. If it
weren't, the current month would appear twice (once from actuals, once
from the generator) and fan out duplicate rows in STEP 14.
===================================================================== */


CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_FUTURE_ACTUAL_MONTHS_v12 AS
 
WITH
/* modeled material universe */
modeled_mtrl AS (
    SELECT DISTINCT mtrl_num
    FROM uspd_analytics_den.analytics_gold.WAC_PI_BT_NDC_MAGNITUDE_v12
),
...
actual_months AS (
    ...
    WHERE r.run_id <> 'FC_LIVE'
),
 
/* ---- NEW BRANCH: 60-month generator ---- */
month_offsets AS (                                   -- CHANGED: new CTE
    SELECT EXPLODE(SEQUENCE(0, 59)) AS n 
),
 
future_months AS (                                   -- CHANGED: new CTE
    SELECT
        r.run_id,
        r.jump_off_month,
        u.mtrl_num,
...
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_FORECASTED_v12 AS
...
WITH base AS (
    ...
    FROM uspd_analytics_den.analytics_gold.WAC_PI_BT_BASELINE_RESOLVED_v12 br
    LEFT JOIN uspd_analytics_den.analytics_gold.WAC_PI_BT_FUTURE_ACTUAL_MONTHS_v12 fm
...
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_EVAL_DETAIL_v12 AS
...
WITH forecast_base AS (
    SELECT
        ...
        f.forecasted_wac
    FROM uspd_analytics_den.analytics_gold.WAC_PI_BT_FORECASTED_v12 f
),
...
attrs AS (
    SELECT DISTINCT
        ...
        therapeutic_class,
        manufacturer_name
    FROM uspd_analytics_den.analytics_gold.WAC_PI_BT_ATTRS_DEDUP_v12
),
...
joined AS (
    ...
    LEFT JOIN uspd_analytics_den.analytics_gold.WAC_PI_BT_BASELINE_RESOLVED_v12 br
    ...
    LEFT JOIN uspd_analytics_den.analytics_gold.WAC_PI_BT_ACTUAL_WAC_MONTHLY_v12 aw
...
---------------------------------------------------------------------------------------
-- FINAL TABLE: EVAL v12 WITH DECREASE FLAGS (FULL BUILD)
----------------------------------------------------------------------------------------
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.WAC_PI_BT_EVAL_v12_WITH_DECREASE_FLAG AS
...
/* =========================================================
STEP 4: BASE FORECAST/EVAL TABLE (v12)
========================================================= */

eval_base AS (
    SELECT *
    FROM uspd_analytics_den.analytics_gold.WAC_PI_BT_EVAL_DETAIL_v12
),
...
FROM uspd_analytics_den.analytics_gold.WAC_PI_BT_EVAL_v12_WITH_DECREASE_FLAG
WHERE run_id = 'FC_LIVE'
;

-- select * from  uspd_analytics_den.analytics_gold.WAC_PI_BT_EVAL_v12_WITH_DECREASE_FLAG
