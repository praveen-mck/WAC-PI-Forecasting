...
vstx AS (
    ...
    FROM (
        SELECT *,
               ROW_NUMBER() OVER (PARTITION BY EM_ITEM_NUM ORDER BY EM_ITEM_NUM) AS rn
...
base AS (
    SELECT
        ...
        /* COMMON GROUP padded to 6 (ZERO SAFE) */
        LPAD(
        ...
        CASE
            ...
            WHEN t_copa.SLS_CTGRY_CD IN ('102','103','106','107','112','116','122','123','701','703','711','806','807','816')
                THEN 'DROP SHIP'
        ...
        /* THERAPEUTIC CLASS FIX (with fallback) */
        COALESCE(
...
filtered AS (
    SELECT * FROM base WHERE CUST_SEGMENT <> 'INTERCO'
),
