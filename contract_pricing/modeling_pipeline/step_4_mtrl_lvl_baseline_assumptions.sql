-- =========================================================
-- STEP 4: MATERIAL-LEVEL BASELINE ASSUMPTIONS v21
--
-- Changes from v20:
-- groupby_key replaces sap_cust_num_trim+mtrl_num everywhere
--   - Source updated to contract_price_training_clean_v21
--   - Output table renamed to contract_price_material_assumptions_v21
--   - HYBRID_MODEL_KEY_3T replaced with sap_cust_num_trim
--     as the series key in ALL CTEs:
--       last_month, anchor_row, calendar_window_agg,
--       ranked_history, latest_6_observed_agg,
--       quarterly_months, quarterly_weighted_price,
--       yearly_months, yearly_weighted_price,
--       yearly_yoy, trend_direction, regression_inputs,
--       trend_regression, guardrails, combined,
--       and the final SELECT/JOIN
--   - MODEL_TIER removed from anchor_row (no tier in v21);
--     sap_months and l2_months retained as informational
--   - All GROUP BY, PARTITION BY, JOIN ON clauses updated
--
--   Carried forward from v20 (logic unchanged):
--   - APOLLO → NO_TREND override
--   - BX baseline upper-bound guard (non-GX: recent_6m/anchor <= 1.5)
--   - avg_yoy_pct naming
...
CREATE OR REPLACE TABLE uspd_analytics_den.analytics_gold.contract_price_material_assumptions_v21 AS
 
WITH base AS (
    SELECT *
    FROM uspd_analytics_den.analytics_gold.contract_price_training_clean_v21
...
anchor_row AS (
    SELECT
        ...
        x.common_grp_id,
        x.common_grp_desc,
        x.subset_l2_id_resolved,
        x.subset_l2_desc_resolved,
        x.mtrl_nme_nvgton,
        x.ndc_num,
        x.product_family,
        x.therapeutic_class,
...
calendar_window_agg AS (
    ...
    FROM last_month lm
    LEFT JOIN base b ON lm.groupby_key = b.groupby_key
    GROUP BY lm.groupby_key, lm.anchor_month
),
...
quarterly_months AS (
    ...
    FROM base b
    INNER JOIN last_month lm ON b.groupby_key = lm.groupby_key
...
quarterly_weighted_price AS (
    ...
    GROUP BY groupby_key, anchor_month, qtr_bucket
),
...
yearly_weighted_price AS (
    ...
    FROM yearly_months
    GROUP BY groupby_key, anchor_month, yr_bucket
),
...
regression_inputs AS (
    SELECT
        ...
        qtr_price,
        MAX(qtr_bucket) OVER (
            PARTITION BY groupby_key, anchor_month
        ) - qtr_bucket                                  AS x
...
guardrails AS (
    SELECT
        ...
        CASE
            WHEN COALESCE(td.yoy_pairs_used, 0) < 2                            THEN 0
            WHEN COALESCE(td.directional_consistency, 0) < 0.60                THEN 0
            WHEN cwa.recent_6m_avg_contract_price IS NULL
              OR cwa.recent_6m_avg_contract_price = 0                          THEN 0
            WHEN ABS(ar.anchor_contract_price
                / NULLIF(cwa.recent_6m_avg_contract_price, 0) - 1) > 0.20      THEN 0
            ELSE 1
        END                                             AS sign_only_eligible,
 
        CASE
            WHEN COALESCE(td.yoy_pairs_used, 0) = 0    THEN 'FAIL_NO_HISTORY'
            WHEN COALESCE(td.yoy_pairs_used, 0) < 2    THEN 'FAIL_INSUFFICIENT_PAIRS'
            WHEN COALESCE(td.directional_consistency, 0) < 0.60 THEN 'FAIL_NOISY_DIRECTION'
            WHEN cwa.recent_6m_avg_contract_price IS NULL
              OR cwa.recent_6m_avg_contract_price = 0  THEN 'FAIL_NO_BASELINE'
            WHEN ABS(ar.anchor_contract_price
                / NULLIF(cwa.recent_6m_avg_contract_price, 0) - 1) > 0.20 THEN 'FAIL_PRICE_JUMP'
            ELSE 'PASS'
        END                                             AS trend_suppression_reason
 
    FROM calendar_window_agg cwa
    LEFT JOIN trend_direction      td ON cwa.groupby_key = td.groupby_key
    LEFT JOIN anchor_row           ar ON cwa.groupby_key = ar.groupby_key
    LEFT JOIN latest_6_observed_agg l6 ON cwa.groupby_key = l6.groupby_key
),
...
combined AS (
    SELECT
        ...
        COALESCE(tr.ols_slope / NULLIF(tr.avg_price_ref, 0), 0)
                                                        AS raw_regression_trend_pct
 
    FROM calendar_window_agg cwa
    LEFT JOIN latest_6_observed_agg l6 ON cwa.groupby_key = l6.groupby_key
    LEFT JOIN guardrails            gr ON cwa.groupby_key = gr.groupby_key
    LEFT JOIN trend_regression      tr ON cwa.groupby_key = tr.groupby_key
)
 
SELECT
    c.groupby_key,
    ar.sap_cust_num_trim,
    ar.mtrl_num,
    ar.sap_months,
    ar.l2_months,
    ...
    ar.common_grp_id,
    ar.common_grp_desc,
    ar.subset_l2_id_resolved,
    ar.subset_l2_desc_resolved,
    ar.mtrl_nme_nvgton,
    ar.ndc_num,
    ar.product_family,
...
FROM combined c
LEFT JOIN anchor_row ar ON c.groupby_key = ar.groupby_key
;
