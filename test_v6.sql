...
/* ---------------------------------------------------------------------
   List of Tables used for defining Universe
   --------------------------------------------------------------------- */

-- Table replicating FDP COPA transaction history
select * from PRD_MT_BIG_BETS_DB.POC.t_pharma_profitability_actuals_fpa_0525 limit 100
--Table derived from FDP COPA
select * from PRD_MT_BIG_BETS_DB.POC.WAC_PI_FORECAST_BASELINE_PRICE_V2_FINAL_MODIFIED limit 100
--Snowflake Item Price History
select * from PRD_PSAS_DB.RPT.T_ITEM_PRC_HIST price_hist limit 100
--Snowflake Item Master Table
select * from "PRD_PSAS_DB"."RPT"."T_IW_EM_ITEM" limit 100
--FDP COPA Dataset - Copy of top records 
select * from DEV_MT_BIG_BETS_DB.POC.T_PHARMA_PROFITABILITY_ACTUALS_FPA limit 100
--Metadata for FDP COPA Dataset Materials 
select * from DEV_MT_BIG_BETS_DB.POC.T_Q_MATERIAL_PHARMA_BW limit 100
...
select * from 
DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_UNIVERSE_v6
where mtrl_num in ('000000000002617694')


select * from 
PRD_MT_BIG_BETS_DB.POC.WAC_PI_FORECAST_BASELINE_PRICE_V2_FINAL_MODIFIED_0603
where COPA_MTRL_NUM in ('000000000002617694')
...
FROM DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_UNIVERSE_v6 u
...
from
    DEV_MT_BIG_BETS_DB.POC.WAC_PI_BF_ATTRS_v6 attrs_v1
left join
    EP
ON
...
WITH ranked AS (
    ...
    FROM DEV_MT_BIG_BETS_DB.POC.WAC_PI_BF_ATTRS_UPD_v6 a
)
SELECT
    ndc_nmbr,
    mtrl_num,
...
select * from  PRD_MT_BIG_BETS_DB.POC.WAC_PI_FORECAST_BASELINE_PRICE_V2_FINAL_MODIFIED_0603
where COPA_MTRL_NUM in ('000000000003229085') order by wac_price_copa_date

select count(distinct mtrl_num) from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_ACTUAL_WAC_MONTHLY_v6 
--6,794
/* =====================================================================
...
WITH base AS (
    ...
    JOIN
        DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_UNIVERSE_v6 mat_univ
    ON
...
select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_MTRL_HISTORY_PROFILE_v6 order by mtrl_num
--6794
...
FROM 
    DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_ACTUAL_WAC_MONTHLY_v6 aw
GROUP BY 1,2
;


select count(distinct mtrl_num) from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_ACTIVE_PRODUCTS_v6 
--6,794
...
select count(distinct mtrl_num) from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_RUN_ELIGIBILITY_v6
--6,794
--5,830

select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_RUN_ELIGIBILITY_v6 order by mtrl_num

select run_id,is_eligible_for_run, count(distinct mtrl_num)
from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_RUN_ELIGIBILITY_v6 
group by 1,2
...
WITH base AS (
    ...
    FROM DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_RUNS_v6 r
    JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_UNIVERSE_v6 u
      ON 1 = 1
    LEFT JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_MTRL_LIFECYCLE_v6 lc
      ON u.ndc_nmbr = lc.ndc_nmbr
     AND u.mtrl_num = lc.mtrl_num
    LEFT JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_ACTIVE_PRODUCTS_v6 ap
...
select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_HIST_v6 order by run_id, mtrl_num, cal_month_start_dt

select run_id, data_coverage_flag, count(distinct mtrl_num) from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_HIST_v6
group by 1,2

select count(distinct mtrl_num) from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_HIST_v6 
5,786
/* =====================================================================
...
FROM DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_RUN_ELIGIBILITY_v6 re
JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_ACTUAL_WAC_MONTHLY_v6 aw
...
-- FROM DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_HIST_v6
-- GROUP BY run_id, mtrl_num
-- ;
-- select count(distinct mtrl_num) from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_HIST_AGE_v6 
-- --5,786


select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_HIST_AGE_v6
...
FROM DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_RUN_ELIGIBILITY_v6 re
LEFT JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_HIST_v6 h
...
select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_LAST_WAC_v6 order by run_id, mtrl_num

select count(distinct mtrl_num) from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_LAST_WAC_v6
--5901
/* =====================================================================
STEP 6: LAST OBSERVED WAC
===================================================================== */
...
select median(price_change_pct) from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_PRICE_CHANGES_v6
where price_change_pct>0

select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_PRICE_CHANGES_v6 order by mtrl_num
...
--         ) AS prev_price
--     FROM DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_HIST_v6 h
-- ) z
-- ;


select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_PRICE_CHANGES_v6 order by run_id, mtrl_num, cal_month_start_dt

select count(distinct mtrl_num) from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_PRICE_CHANGES_v6
--5786
...
select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_PRICE_CHANGES_v6 where price_change_pct < 0 order by run_id, cal_month_start_dt desc, mtrl_num

select count(distinct mtrl_num) from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_PRICE_CHANGES_v6 where price_change_pct < 0 order by run_id, cal_month_start_dt desc, mtrl_num
--308

select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_PRICE_CHANGES_v6 where price_change_pct < 0 order by run_id desc , cal_month_start_dt desc, mtrl_num
--308

select run_id, cal_month_start_dt, count(*) 
from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_PRICE_CHANGES_v6 where price_change_pct < 0  
group by 1,2
...
select low_price_change_outlier_flag, high_price_change_outlier_flag, count(distinct mtrl_num) from  DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_PRICE_CHANGES_v6
group by 1,2

select low_base_price_flag , count(distinct mtrl_num) from  DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_PRICE_CHANGES_v6
group by 1
...
select count(distinct mtrl_num) from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_VALID_INCREASE_EVENTS_v6 
--3413
...
FROM DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_VALID_INCREASE_EVENTS_v6;
...
select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_EVENT_PROFILE_v6 order by run_id, mtrl_num

select count(distinct mtrl_num) from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_EVENT_PROFILE_v6 
--5901
/* =====================================================================
...
FROM DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_RUN_ELIGIBILITY_v6 re
LEFT JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_PRICE_CHANGES_v6 pc
...
select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_LAST_INCREASE_v6 order by run_id, mtrl_num

select count(distinct mtrl_num) from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_LAST_INCREASE_v6
--3413
/* =====================================================================
...
--     FROM DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_VALID_INCREASE_EVENTS_v6 fi
...
loe_stats AS (
    ...
    FROM DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_PRICE_CHANGES_v6 pc
    LEFT JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BF_ATTRS_UPD_v6 loe
...
select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_NDC_TIMING_v6 order by mtrl_num, run_id
...
WITH material_level AS (
    ...
    FROM DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_RUN_ELIGIBILITY_v6 re
    JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_ATTRS_DEDUP_v6 ad
      ON re.ndc_nmbr = ad.ndc_nmbr
     AND re.mtrl_num = ad.mtrl_num
    LEFT JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_NDC_MAGNITUDE_v6 nm
      ON re.run_id = nm.run_id
     AND re.mtrl_num = nm.mtrl_num
    LEFT JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_NDC_TIMING_v6 nt
...
select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_FALLBACK_ENRICHED_v6 order by run_id, mtrl_num

select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_FALLBACK_ENRICHED_v6 where fallback_priority = 'NO THERAP/MFR AVAILABLE'
...
JOIN ATTRIBUTES
=============================== */
LEFT JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_ATTRS_DEDUP_V6 a
...
=============================== */
LEFT JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_THERAP_FALLBACK_V6 tf
...
=============================== */
LEFT JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_MFR_FALLBACK_V6 mf
  ON base.run_id = mf.run_id
 AND a.manufacturer_name = mf.manufacturer_name
;

-- select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_BASELINE_RESOLVED_v6 order by run_id, mtrl_num, cal

-- select count(distinct mtrl_num) from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_BASELINE_RESOLVED_v6
-- --5786


select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_BASELINE_RESOLVED_v6 where price_change_pct < 0 order by run_id, cal_month_start_dt desc, mtrl_num


/* =====================================================================
STEP 12d: BASELINE RESOLVED ASSUMPTIONS
- Combines Step-10 magnitude + Step-11 timing
- Uses FALLBACK_ENRICHED_v6 only for NEW products (<12 months)
...
joined AS (
    ...
    LEFT JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_MODEL_INPUT_STATS_v6 ms
        ON bm.run_id = ms.run_id
       AND bm.mtrl_num = ms.mtrl_num
    LEFT JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_NDC_TIMING_v6 nt
        ON bm.run_id = nt.run_id
       AND bm.mtrl_num = nt.mtrl_num
    LEFT JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_FALLBACK_ENRICHED_v6 fe
        ON bm.run_id = fe.run_id
       AND bm.mtrl_num = fe.mtrl_num
    LEFT JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_ATTRS_DEDUP_v6 attr
...
select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_FUTURE_ACTUAL_MONTHS_v6 order by run_id, mtrl_num, forecast_month
...
JOIN (
    ...
    FROM DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_NDC_MAGNITUDE_v6
) u
...
select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_FORECASTED_v6  where mtrl_num = '000000000002013282' order by run_id, mtrl_num, forecast_month


select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_FORECASTED_v6 order by run_id, mtrl_num, forecast_month

select count(distinct mtrl_num) from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_FORECASTED_v6
--5786
...
WITH base AS (
    ...
    FROM DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_BASELINE_RESOLVED_v6 br
    LEFT JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_FUTURE_ACTUAL_MONTHS_v6 fm
...
FROM DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_BASELINE_RESOLVED_v6
GROUP BY
    run_id,
    forecast_source
ORDER BY
...
SELECT
    ...
    n_expected_increases,
    COUNT(*) AS row_cnt
FROM DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_FORECASTED_v6
GROUP BY
    run_id,
    n_expected_increases
ORDER BY
...
from
DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_EVAL_DETAIL_v6 
group by 
1

select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_EVAL_DETAIL_v6 order by run_id, mtrl_num, forecast_month
...
WITH forecast_base AS (
    SELECT
        ...
        f.forecasted_wac
    FROM DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_FORECASTED_v6 f
),
...
attrs AS (
    SELECT DISTINCT
        ...
        therapeutic_class,
        manufacturer_name
    FROM DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_ATTRS_DEDUP_v6
),
...
joined AS (
    ...
    LEFT JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_BASELINE_RESOLVED_v6 br
    ...
    LEFT JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_ACTUAL_WAC_MONTHLY_v6 aw
...
select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_EVAL_DETAIL_v6

select count(distinct mtrl_num) from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_EVAL_DETAIL_v6

select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_EVAL_DETAIL_v6 where forecast_new is null



select * from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_EVAL_SUMMARY_RUN_v6 

select run_id, row_cnt,material_cnt, wac_wape_new, dollar_wape_new, mape_new_wac, mape_new_dollars
from
DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_EVAL_SUMMARY_RUN_v6 

/* =====================================================================
Step 15b. DECREASE FLAG TABLE
- Flags materials that show any actual WAC decrease in history
- Intended for dashboard exclusion / evaluation filtering
===================================================================== */

CREATE OR REPLACE TABLE DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_DECREASE_FLAGS_v6 AS
...
select count(distinct mtrl_num) from DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_EVAL_V6_WITH_DECREASE_FLAG where has_price_decrease_flag > 0
...
CREATE OR REPLACE TABLE DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_EVAL_V6_WITH_DECREASE_FLAG AS
...
FROM DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_EVAL_DETAIL_v6 e
LEFT JOIN DEV_MT_BIG_BETS_DB.POC.WAC_PI_BT_DECREASE_FLAGS_v6 d
